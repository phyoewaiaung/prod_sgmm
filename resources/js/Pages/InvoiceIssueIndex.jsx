...
const InvoiceIssueIndex = (props) => {
    ...
    const [handlingFee, setHandlingFee] = useState(
        props.data.handling_fee == "1" ? true : false
    );
    const [categories, setCategories] = useState([]);
    const [foodUnit, setFoodUnit] = useState();
    const [clothUnit, setClothUnit] = useState();
    ...
    //custom unit
    const [foodUnitCustom, setFoodUnitCustom] = useState();
    const [clothUnitCustom, setClothUnitCustom] = useState();
    const [shoeUnitCustom, setShoeUnitCustom] = useState();
    const [frozenUnitCustom, setFrozenUnitCustom] = useState();
    const [cosmeticUnitCustom, setCosmeticUnitCustom] = useState();
    const [electronicUnitCustom, setElectronicUnitCustom] = useState();
    ...
    const [othersUnit, setOthersUnit] = useState(0.0);
    ...
    useEffect(() => {
        ...
        const foodUnit = props.data.shipment_method
            ? props.data.shipment_method == "1"
                ...
                ? "3.00"
                : "8"
            : props.data.transport == "1"
            ? parseFloat(foodWeight) > 20
                ? "4.00"
                : "4.50"
            : "8.00";
    ...
    const foodWeightChange = (e) => {
        if (!e.target.value) {
            ...
            setTotalFood((parseFloat(e.target.value) * (foodUnitCustom ? foodUnitCustom : foodUnit)).toFixed(2));
        }
    };
    const clothWeightChange = (e) => {
        if (!e.target.value) {
            ...
            setTotalCloth((parseFloat(e.target.value) * (clothUnitCustom ? clothUnitCustom : clothUnit)).toFixed(2));
        }
    };
    const frozenWeightChange = (e) => {
        if (!e.target.value) {
            ...
            setTotalFrozen(
                (parseFloat(e.target.value) * (frozenUnitCustom ? frozenUnitCustom : frozenUnit)).toFixed(2)
            );
        }
    };
    const shoeWeightChange = (e) => {
        if (!e.target.value) {
            ...
            setTotalShoe((parseFloat(e.target.value) * (shoeUnitCustom ? shoeUnitCustom : shoeUnit)).toFixed(2));
        }
    };
    const cosmeticWeightChange = (e) => {
        if (!e.target.value) {
            ...
            setTotalCosmetic(
                (parseFloat(e.target.value) * (cosmeticUnitCustom ? cosmeticUnitCustom : cosmeticUnit)).toFixed(2)
            );
        }
    };
    const electronicWeightChange = (e) => {
        if (!e.target.value) {
            ...
            setTotalElectronic(
                (parseFloat(e.target.value) * (electronicUnitCustom ? electronicUnitCustom : electronicUnit)).toFixed(2)
            );
        }
    };
    ...
    const othersUnitChange = (e) => {
        if (!e.target.value) {
            ...
            setTotalOther(
                (parseFloat(e.target.value) * parseFloat(othersWeight)).toFixed(
                    2
                )
            );
        }
    ...
    const updateClick = (mail) => {
        ...
        let params = {
                ...
                howInSg == undefined
                    ? sgPickup == "1" &&
                      (foodWeight == "" ? 0.0 : parseFloat(foodWeight)) +
                          ...
                          (cosmeticWeight == ""
                              ? 0.0
                              : parseFloat(cosmeticWeight)) +
                          (electronicWeight == ""
                              ? 0.0
                              : parseFloat(electronicWeight)) +
                          (othersWeight == ""
                              ? 0.0
                              : parseFloat(othersWeight)) >=
                          7
                        ? 4.9
                        : 7.0
                    : "3.5",
            ...
            collection_type:
                howInSg == undefined
                    ? howInYgn == "1"
                        ? "Yangon Home Delivery Downtown ($3.5)"
                        : howInYgn == "2"
                        ? "Yangon Home Deliver outside ($5.0)"
                        : howInYgn == "3"
                        ? "Bus Gate ($3.5)"
                        : "Self Collection"
                    : howInSg == "1"
                    ? "SG Home Delivery (S$5.90 within two days)"
                    : howInSg == "2"
                    ? "SG Home Delivery (S$10.0 within one day)"
                    : "Self Collection",
        ...
        setLoading(true);
        let text = mail
            ? "Successfully Sent Email And Update!"
            : "Successfully Update!";
        axios
            .post("/save-issue", params)
            .then((res) => {
                setLoading(false);
                toast.success(text, {
                    position: "top-right",
    ...
    const foodEditClick = (d) => {
        ...
        if(!d.editStatus && foodUnitCustom) {
            setTotalFood((foodUnitCustom * foodWeight).toFixed(2));
        }else if(!d.editStatus && !foodUnitCustom){
            setTotalFood((foodUnit * foodWeight).toFixed(2));
        }
    };

    const clothEditClick = (d) => {
        setCategories(
            categories.map((data) => {
                if (data.id === d.id) {
                    data.editStatus = !data.editStatus;
        ...
        if(!d.editStatus && clothUnitCustom) {
            setTotalCloth((clothUnitCustom * clothWeight).toFixed(2));
        }else if(!d.editStatus && !clothUnitCustom){
            setTotalCloth((clothUnit * clothWeight).toFixed(2));
        }
    };

    const shoeEditClick = (d) => {
        setCategories(
            categories.map((data) => {
                if (data.id === d.id) {
                    data.editStatus = !data.editStatus;
        ...
        if(!d.editStatus && shoeUnitCustom) {
            setTotalShoe((shoeUnitCustom * shoeWeight).toFixed(2));
        }else if(!d.editStatus && !shoeUnitCustom){
            setTotalShoe((shoeUnit * shoeWeight).toFixed(2));
        }
    };

    const frozenEditClick = (d) => {
        setCategories(
            categories.map((data) => {
                if (data.id === d.id) {
                    data.editStatus = !data.editStatus;
        ...
        if(!d.editStatus && frozenUnitCustom) {
            setTotalFrozen((frozenUnitCustom * frozenWeight).toFixed(2));
        }else if(!d.editStatus && !frozenUnitCustom){
            setTotalFrozen((frozenUnit * frozenWeight).toFixed(2));
        }
    };

    const cosmeticEditClick = (d) => {
        setCategories(
            categories.map((data) => {
                if (data.id === d.id) {
                    data.editStatus = !data.editStatus;
        ...
        if(!d.editStatus && cosmeticUnitCustom) {
            setTotalCosmetic((cosmeticUnitCustom * cosmeticWeight).toFixed(2));
        }else if(!d.editStatus && !cosmeticUnitCustom){
            setTotalCosmetic((cosmeticUnit * cosmeticWeight).toFixed(2));
        }
    };

    const electronicEditClick = (d) => {
        setCategories(
            categories.map((data) => {
                if (data.id === d.id) {
                    data.editStatus = !data.editStatus;
        ...
        if(!d.editStatus && electronicUnitCustom) {
            setTotalElectronic((electronicUnitCustom * electronicWeight).toFixed(2));
        }else if(!d.editStatus && !electronicUnitCustom){
            setTotalElectronic((electronicUnit * electronicWeight).toFixed(2));
        }
    };

    const foodUnitChange = (e) => {
        if (isdigit(e.target.value)) {
            setFoodUnitCustom(e.target.value);
        } else {
            setFoodUnitCustom("");
        }
    };

    const clothUnitChange = (e) => {
        if (isdigit(e.target.value)) {
            setClothUnitCustom(e.target.value);
        } else {
            setClothUnitCustom("");
        }
    };

    const shoeUnitChange = (e) => {
        if (isdigit(e.target.value)) {
            setShoeUnitCustom(e.target.value);
        } else {
            setShoeUnitCustom("");
        }
    };

    const frozenUnitChange = (e) => {
        if (isdigit(e.target.value)) {
            setFrozenUnitCustom(e.target.value);
        } else {
            setFrozenUnitCustom("");
        }
    };

    const cosmeticUnitChange = (e) => {
        if (isdigit(e.target.value)) {
            setCosmeticUnitCustom(e.target.value);
        } else {
            setCosmeticUnitCustom("");
        }
    };

    const electronicUnitChange = (e) => {
        if (isdigit(e.target.value)) {
            setElectronicUnitCustom(e.target.value);
        } else {
            setElectronicUnitCustom("");
        }
    };
            ...
            <div className="relative pt-6 pb-6 sm:flex sm:justify-center flex-col sm:items-center min-h-screen bg-dots-darker bg-center bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white">
                    ...
                    <div className="dark:text-gray-400 mt-[-78px] md:mt-0">
                        {props.data.form == "1" ? (
                            <>
                                ...
                                <h4>
                                    111 North Bridge Road, #02-02A, Peninsula
                            ...
                            <>
                                ...
                                <h4>Contact: +65 9325 0329</h4>
                                <h4>
                                    No.642 Thanthumar Road, 10 Qtr, South
                                    Okkalapa, Yangon
                                </h4>
                                <h4>Contact: +959 962 507 694</h4>
                                <br />
                ...
                <main className="md:w-5/6 w-full">
                    ...
                    <div className="flex md:flex-row flex-col md:justify-evenly mt-3">
                        ...
                        <div>
                            <h3 className="font-bold text-center dark:text-gray-400">
                                Delievery Mode :{" "}
                                <span className="font-normal">
                                    {transport
                                        ? transport == "1"
                    ...
                    <div className="flex md:flex-row flex-col md:justify-evenly mt-3">
                        <div className="md:w-1/2 w-full text-center">
                            <h3 className="font-bold text-center md:text-start ml-11 dark:text-blue-500">
                    ...
                    <div className="mt-4 ml-3 mr-3 md:ml-0 md:mr-0">
                        <h3 className="dark:text-red-400 font-bold">
                            {senderEmail}
                        </h3>
                    </div>
                        ...
                        <table className="invoice-issue-table text-center">
                            ...
                            <tbody className="dark:text-gray-400">
                                            ...
                                            <React.Fragment key={index}>
                                                {data.id == "1" && (
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <input
                                                                className="w-1/2 dark:bg-gray-400 dark:text-white"
                                                                type="text"
                                                                value={
                                                                    foodWeight ===
                                                                    0
                                                        ...
                                                        <td>
                                                            <div className="relative">
                                                                ...
                                                                <div
                                                                    ...
                                                                    className={
                                                                        data.editStatus
                                                                            ? "absolute top-[4px] right-[0] cursor-pointer bg-green-200 hover:bg-green-400 p-1 rounded  hover:text-green-800"
                                                                            : "absolute top-[-4px] right-[0] cursor-pointer hover:text-blue-800 hover:bg-gray-300  p-1 rounded"
                                                                    }
                                                                >
                                                                    {data.editStatus ? (
                                                                        <CheckIcon />
                                                            ...
                                                            <div className="relative">
                                                                {data.editStatus ? (
                                                                    <input
                                                                        ...
                                                                        placeholder="new unit price"
                                                                        value={
                                                                            electronicUnitCustom
                                                                        }
                                                                        onChange={
                                                                            electronicUnitChange
                                                                        }
                                                                    />
                                                                ) : (
                                                                    <Typography>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {(
                                            (foodWeight == ""
                                                ? 0.0
                                                : parseFloat(foodWeight)) +
                                            (clothWeight == ""
                                                ? 0.0
                                                : parseFloat(clothWeight)) +
                                            (frozenWeight == ""
                                                ? 0.0
                                                : parseFloat(frozenWeight)) +
                                            (shoeWeight == ""
                                                ? 0.0
                                                : parseFloat(shoeWeight)) +
                                            (cosmeticWeight == ""
                                                ? 0.0
                                                : parseFloat(cosmeticWeight)) +
                                            (electronicWeight == ""
                                    ...
                                    <td>
                                        <input
                                            className="cursor-pointer focus:ring focus:ring-blue-100 focus:ring-opacity-50 focus:outline-none"
                                            type="checkbox"
                                            value={handlingFee}
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {howInSg == undefined
                                            ? "Home/ Bus Station deliver:"
                                            : "SG Home Delivery / Self Collection:"}
                                    </td>
                                    <td className="text-start" colSpan={2}>
                                        {howInSg == undefined
                                            ? howInYgn == "1"
                                ...
                                <tr>
                                    ...
                                    <td colSpan={2} className="font-bold">
                                        PayNow to mobile 93250329 or UEN number
                                        53413642K{" "}
                                    </td>
                                    <td>TOTAL</td>
                                    <td>
                    ...
                    <h3 className="ml-3 mr-3 md:mr-0 md:ml-0 font-bold text-red-600 mt-1">
                        *Any Loss or Damage, we will refund item price OR refund
                        3 times of shipping fee (item 1 to 5 only), whichever is
                        lower.
                    </h3>
                    <h3 className="ml-3 mr-3 md:mr-0 md:ml-0 font-bold text-red-600">
                        *if require full refund, additional 5% of item value
                        have to pay upfront
                    </h3>
                    <div className="ml-5 mr-5 md:mr-0 md:ml-0 flex md:flex-row flex-col md:justify-between mb-5 items-center">
                        <div className="dark:text-gray-400">
                            ...
                            {props.data.form == "1" ? (
                                <ol>
                                    <li>
                                        All prices stated here are in Singapore
                                        Dollars
                                    </li>
                                    <li>
                                        Any illegal items will not be accepted
                                    </li>
                                    <li>
                                        {" "}
                                        Arrival schedule might change due to
                                ...
                                <ol>
                                    ...
                                    <li>
                                        Flight schedule might change due to
                                        unforeseen circumstances
                                    </li>
                                    <li>
                                        Due to tighter restriction, offload is
                                        expected to happen
                                    </li>
                                    <li>
                                        Please provide full address for SG Home
                                        Delivery
                                    </li>
                                    <li>
                                        ALL FROZEN FOOD to be packed and sealed
                                        properly
                                    </li>
                                    <li>
                                        Any Loss or Damage (except FROZEN FOOD)
                                        will be refunded 3 times of shipping
                                        fees{" "}
                                    </li>
                                </ol>
                            )}
                        ...
                        <div className="md:mr-[80px] md:mt-0 mt-4">
                            <div>
                                <button
                                    onClick={() => updateClick(true)}
                                    className="invoice-issue-button"
                                >
                                    Send Email
                                </button>
                            </div>
